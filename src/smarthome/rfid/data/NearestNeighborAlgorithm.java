...
public class NearestNeighborAlgorithm implements Algorithm {
	...
	public Location getLocation(int tagId, Vector signalStrength) {
		float weight = 0; 
		float smallest = (float) RFIDMath.getWeight(signalStrength.data(), trainingData.get(0).signalStrength().data());
		int index = 0; 
		
		for (int i =1; i < Math.min(signalStrength.size(), trainingData.size()); i++) {
			weight = (float) RFIDMath.getWeight(signalStrength.data(), trainingData.get(i).signalStrength().data());
			if (weight <= smallest) {
				smallest = weight; 
				index = i; 
			}
		...
		return trainingData.get(index).location();
	}
